...
from tools.input_parse import PoboljsajUnos
...
def main(page: ft.Page):
    ...
    def dodajulistu(pitanje, odgovor, kontekst, aplikacija):
        if not pitanje or not odgovor or not kontekst or not aplikacija:
            show_message("Molimo unesite sve podatke uključujući aplikaciju.", ft.Colors.RED)
            return
    ...
    def Poboljsaj(pitanje, odgovor, kontekst):
        if not pitanje or not odgovor or not kontekst:
            show_message("Molimo unesite pitanje, odgovor i kontekst za poboljšanje.", ft.Colors.RED)
            return
        rezultat = PoboljsajUnos(pitanje, odgovor, kontekst)
        print(f"AI PoboljšanjeEEEEEEE: {rezultat}")
        pitanjeUnos.value = rezultat['pitanje']
        odgovorUnos.value = rezultat['odgovor']
        show_message("AI poboljšanje uspješno!", ft.Colors.GREEN)
        ucitaj_df()
    ...
    def confirm_upload(e):
        ...
        try:
            ...
            show_message("Uspješno poslano u bazu!", ft.Colors.GREEN)
        except Exception as ex:
            show_message(f"Greška pri slanju u bazu: {ex}", ft.Colors.RED)
    ...
    def ucitaj_df():
        ...
        try:
            ...
            show_message(f"Greška pri učitavanju tabele: {e}", ft.Colors.RED)

    page.title = "Chatbot Unos Podataka"
    Title = ft.Row([
        ft.Text(
            "IP Chatbot",
    ...
    ], alignment="left", vertical_alignment="end")  
    ...
    DodajUListuGumb = ft.Row([
        ft.ElevatedButton("Dodaj u listu", on_click=lambda e: dodajulistu(pitanjeUnos.value, odgovorUnos.value, KontekstUnos.value, aplikacijaUnos.value)),
        ft.ElevatedButton("AI Poboljsaj Unos", on_click=lambda e: Poboljsaj(pitanjeUnos.value, odgovorUnos.value, KontekstUnos.value)),
...
if __name__ == "__main__":
    ...
    ft.app(target=main, upload_dir="../unos/dokumentacija")